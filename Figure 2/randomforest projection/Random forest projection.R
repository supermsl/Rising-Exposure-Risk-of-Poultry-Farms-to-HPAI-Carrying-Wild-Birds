...
# 
file_names <- c("2033A1Brcp45", "2033A2rcp45", "2033B1rcp45", "2033B2rcp45",
                "2033A1Brcp85", "2033A2rcp85", "2033B1rcp85", "2033B2rcp85",
                "2099A1Brcp45", "2099A2rcp45", "2099B1rcp45", "2099B2rcp45",
                "2099A1Brcp85", "2099A2rcp85", "2099B1rcp85", "2099B2rcp85")
...
for (fname in file_names) {
  #
  file_path <- paste0(fname, ".txt")
  
  #
  new_data <- read.delim(file_path, na.strings = "NA", header = TRUE, fileEncoding = "UTF-16")
  new_data <- na.omit(new_data)
  
  #
  new_data <- new_data[, expected_vars, drop = FALSE]
  
  #
  for (col in expected_vars) {
    if (is.factor(otu[[col]])) {
      new_data[[col]] <- factor(new_data[[col]], levels = levels(otu[[col]]))
    }
  }
  
  #
  pred <- predict(otu_forest, new_data)
  new_data$predicted_American_wigeon <- pred
  
  #
  out_path <- paste0("AWpredicted_", fname, ".csv")
  write.csv(new_data, out_path, row.names = FALSE)
}