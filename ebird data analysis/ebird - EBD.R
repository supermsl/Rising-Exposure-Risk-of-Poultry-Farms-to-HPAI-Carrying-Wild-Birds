...
f_ebd <- "ebd_US_mallar3_202201_202312_smp_relSep-2024.txt"
...
observation_column <- "observation_count"
...
result_2022 <- data %>%
  ...
  summarise(
    total_observation_2022 = sum(.data[[observation_column]], na.rm = TRUE),
    count_2022 = n(),
    .groups = "drop"
  )
...
result_2023 <- data %>%
  ...
  group_by(across(all_of(c(state_number, county_number)))) %>%
  summarise(
    total_observation_2023 = sum(.data[[observation_column]], na.rm = TRUE),
    count_2023 = n(),
    .groups = "drop"
  )
...
result_2022_2023 <- data %>%
  ...
  group_by(across(all_of(c(state_number, county_number)))) %>%
  summarise(
    total_observation_2022_2023 = sum(.data[[observation_column]], na.rm = TRUE),
    count_2022_2023 = n(),
    .groups = "drop"
  )
...
#
write.csv(final_result, 'mallar analysis for 2022, 2023, and 2022-2023.csv', row.names = FALSE, fileEncoding = "UTF-8", quote = TRUE)
